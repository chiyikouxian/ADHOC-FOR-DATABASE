-- =============================================================
-- FANET 平台 — PostgreSQL 关系侧种子数据
-- 执行顺序：schema_pg.sql 之后再执行本文件
-- 遥测历史数据由 S3 遥测模拟器灌入 TDengine，本文件只填关系侧
-- =============================================================
...
INSERT INTO drone_models (model_id, model_name, max_flight_minutes, max_speed) VALUES
    ...
    (2, 'Relay-R2',  50, 12.50),
    (3, 'Heavy-H3',  25, 22.00);
-- 地面站不是飞行器，无型号（drones.model_id 置 NULL），故此处不建“型号”

-- ---------- 3.1 无人机实例（drone_id=0 为地面站）----------
INSERT INTO drones (drone_id, model_id, serial_no, status) VALUES
    (0, NULL, 'GS-0000', 'idle'),     -- 地面站，递归拓扑终点，无型号
    (1, 1, 'SN-0001', 'idle'),
    (2, 1, 'SN-0002', 'flying'),
    (3, 2, 'SN-0003', 'flying'),
...
-- ---------- 3.2 用户（密码为 bcrypt 占位 hash，登录逻辑见 S2）----------
-- 占位 hash 对应明文 "password"，仅供开发；上线前必须改
...
-- ---------- 3.3 历史任务 + 分配 + 航点 ----------
-- creator_id 取 admin=1 / operator1=2（与上方插入顺序对应）
INSERT INTO missions (mission_id, creator_id, title, status, planned_start) VALUES
    (1, 1, '城区边界巡检', 'completed', now() - interval '3 day'),
    (2, 2, '夜间侦察任务', 'running',   now() - interval '2 hour'),
    (3, 2, '中继链路压力测试', 'scheduled', now() + interval '1 day'),
    (4, 1, '应急搜寻演练', 'draft',     NULL);

-- 任务分配（多对多桥表）；任务2在飞，任务1已完成
...
-- 航点路径（任务1 drone1 的巡检航线，按 seq 排序）
...
-- ---------- 3.4 drone_latest 初始快照 ----------
-- 模拟接入服务回写的最新状态（真实数据由 S3 遥测模拟器持续刷新）
INSERT INTO drone_latest (drone_id, ts, lat, lon, alt, battery_pct, rssi) VALUES
    (0, now(), 31.225000, 121.460000,   0.0, 100.0,  -40),   -- 地面站
    ...
    (6, now() - interval '5 min', 31.240000, 121.480000, 0.0, 12.0, -95),  -- 低电量+离线
    (7, now() - interval '1 hour', 31.226000, 121.461000, 0.0, 100.0, -50);

-- ---------- 3.4 链路快照（最近一窗活跃链路，供递归 CTE 演示）----------
-- 拓扑：2→1→0, 3→1→0, 4→3（4 经 3 经 1 到地面站 0）
INSERT INTO network_links_snapshot (src_drone_id, dst_drone_id, link_quality, is_active) VALUES
    ...
    (4, 3, 55, TRUE),
    (5, 0, 88, TRUE),
    (6, 2, 20, FALSE);   -- 失活链路（节点6离线）

-- ---------- 3.4 若干告警 ----------
INSERT INTO alerts (drone_id, alert_type, severity, detail, resolved) VALUES
    (6, 'battery_drop', 'critical', '电量降至 12%，低于安全阈值', FALSE),
    (6, 'link_loss',    'critical', '与上游节点链路失活', FALSE),
    (3, 'battery_drop', 'warning',  '电量 45%，建议返航', FALSE),
    (2, 'gps_drift',    'info',     'GPS 轻微漂移，已自动校正', TRUE);
